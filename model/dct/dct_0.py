...
class DCTNet(nn.Module):
    def __init__(self, layers=50, dropout=0.1, classes=2, use_dct=True, vec_dim=300):
        ...
        if use_dct:
            ...
            self.up_conv = ConvBNReLU(fea_dim, fea_dim, ks=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        if self.use_dct:
            ...
            h = int(np.sqrt(feat_dct.size()[2]))
            # feat_dct = feat_dct.permute(0, 2, 1).contiguous()
            feat_dct_matrice = feat_dct.view(N, -1, h, h)  # DCT vector -> DCT matrice
...
if __name__ == "__main__":
    ...
    model = DCTNet(layers=18,  dropout=0.1, classes=21, vec_dim=16).cuda()
    model.eval()
    print(model)
    output = model(input)
